...
// --- Campos preenchidos pelo próprio portal -------------------------------

// O bloco do emitente vem do cadastro da conta, não de uma escolha de quem
// emite. Parte dele o portal marca como somente leitura — CNPJ, razão social,
// telefone, e-mail, município, inscrição municipal — e essa parte já fica de
// fora. O endereço, porém, é editável: aparece preenchido, aceita digitação e
// seria guardado como se fosse dado da empresa. Guardar isso não ajuda em nada
// e ainda faz o preenchimento reescrever o que o portal acabou de trazer.
const AUTO_FILLED_PATTERNS = [
  /^Prestador\./i
];

export function isAutoFilledField(savedField) {
  return candidateNames(savedField).some(
    (name) => AUTO_FILLED_PATTERNS.some((pattern) => pattern.test(name))
  );
}

export function stripAutoFilledFields(savedForm) {
  const fields = (savedForm?.fields || []).filter((field) => !isAutoFilledField(field));
  if (fields.length === (savedForm?.fields || []).length) return savedForm;
  return { ...savedForm, fields };
}
